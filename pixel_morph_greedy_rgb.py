...
def build_knn_lists_local(src_rgb:np.ndarray,tgt_rgb:np.ndarray,k:int)->np.ndarray:
    try:
        from scipy.spatial import cKDTree
        tree=cKDTree(src_rgb.astype(np.float32),leafsize=32,compact_nodes=True,balanced_tree=True)
        return _tree_query(tree,tgt_rgb.astype(np.float32),k=k)
    except Exception:
        return knn_lists_binned(src_rgb,tgt_rgb,k=k,bins=16)

def greedy_assign_targets_to_sources(neighbors:np.ndarray,importance_order:np.ndarray,n_src:int)->Tuple[np.ndarray,np.ndarray]:
    m,_=neighbors.shape
    ...
    return chosen,np.array(unassigned,dtype=np.int32)

def resolve_unassigned_with_expanding_knn_local(src_rgb:np.ndarray,tgt_rgb:np.ndarray,chosen:np.ndarray,unassigned:np.ndarray,k_init:int,k_max:int)->None:
    ...
    if not use_tree:
        ...
        for i,t in enumerate(unassigned):
            chosen[t]=remaining[i%remaining.size]
            used[chosen[t]]=True
        return
    ...
    for t in unassigned:
        ...
        picked=-1
        while True:
            nn=_tree_query(tree,tgt_rgb[int(t)][None,:].astype(np.float32),k=k)[0]
            for cand in nn:
                if not used[cand]:
                    picked=int(cand)
                    used[picked]=True
        ...
        if picked==-1:
            rem=np.where(~used)[0]
            if rem.size:
                picked=int(rem[0]); used[picked]=True
        chosen[int(t)]=picked


def chunk_match(src_rgb_u8:np.ndarray,tgt_rgb_u8:np.ndarray,importance:np.ndarray,src_perm:np.ndarray,tgt_perm:np.ndarray,lo:int,hi:int,k_init:int,k_max:int)->Tuple[np.ndarray,np.ndarray]:
    src_ids=src_perm[lo:hi]
    tgt_ids=tgt_perm[lo:hi]

    src_rgb=src_rgb_u8[src_ids].astype(np.float32)
    tgt_rgb=tgt_rgb_u8[tgt_ids].astype(np.float32)

    imp=importance[tgt_ids].astype(np.float32)
    ...
    k0=max(1,min(int(k_init),n_src))
    kM=max(k0,min(int(k_max),n_src))

    neighbors=build_knn_lists_local(src_rgb,tgt_rgb,k=k0)
    chosen_local,unassigned=greedy_assign_targets_to_sources(neighbors,local_order,n_src=n_src)

    if unassigned.size>0:
        resolve_unassigned_with_expanding_knn_local(src_rgb,tgt_rgb,chosen_local,unassigned,k_init=k0,k_max=kM)

    chosen_global=np.full(m,-1,dtype=np.int32)
    ok=chosen_local>=0
    chosen_global[ok]=src_ids[chosen_local[ok]]
    return tgt_ids,chosen_global
...
def main():
    ...
    ap.add_argument("--out",default="segmented_seq.gif")

    ap.add_argument("--jobs",type=int,default=max(1,(os.cpu_count() or 1)),help="number of hue segments (same meaning as parallel --jobs)")
    ...
    print(f"[greedy segmented seq] hue-groups/jobs = {jobs}")
    chosen_src_for_tgt=np.full(M,-1,dtype=np.int32)

    prog=Progress(total=jobs,prefix="[greedy segmented seq]",every=1)
    for i in range(jobs):
        lo=(i*M)//jobs
        hi=((i+1)*M)//jobs
        tgt_ids,src_ids=chunk_match(src_rgb_u8=src_rgb_u8,tgt_rgb_u8=tgt_rgb_u8,importance=importance,src_perm=src_perm,tgt_perm=tgt_perm,lo= lo,hi=hi,k_init=args.k_init,k_max=args.k_max)
        chosen_src_for_tgt[tgt_ids]=src_ids
        prog.tick(i+1)

    if np.any(chosen_src_for_tgt<0):
        bad=int(np.sum(chosen_src_for_tgt<0))
        print(f"[warn] {bad} targets remained unassigned; filling deterministically.")
        used=np.zeros(M,dtype=np.bool_)
        used[chosen_src_for_tgt[chosen_src_for_tgt>=0]]=True
        remaining=np.where(~used)[0].astype(np.int32)
        fill_tgts=np.where(chosen_src_for_tgt<0)[0].astype(np.int32)
        for j,t in enumerate(fill_tgts):
            chosen_src_for_tgt[t]=remaining[j%remaining.size]

    t_match1=time.perf_counter()

    t_map0=time.perf_counter()
    print("[map] building start/end trajectories + end colors...")

    start_xy=src_xy.astype(np.float32)
    fixed_col01=(src_rgb_u8.astype(np.float32)/255.0).astype(np.float32)

    end_xy=np.empty_like(start_xy,dtype=np.float32)
    end_col01=np.empty_like(fixed_col01,dtype=np.float32)

    valid=chosen_src_for_tgt>=0
    end_xy[chosen_src_for_tgt[valid]]=tgt_xy[valid].astype(np.float32)

    alpha=0.35
    end_col01[chosen_src_for_tgt[valid]]=(alpha*(tgt_rgb_u8[valid].astype(np.float32)/255.0)+(1-alpha)*(src_rgb_u8[chosen_src_for_tgt[valid]].astype(np.float32)/255.0))

    if np.any(~valid):
        end_xy[~valid]=start_xy[~valid]
        end_col01[~valid]=fixed_col01[~valid]
    ...
    print(f"  segment:     {(t_seg1 -t_seg0):8.3f}s")
    print(f"  greedy(seg): {(t_match1-t_match0):8.3f}s")
